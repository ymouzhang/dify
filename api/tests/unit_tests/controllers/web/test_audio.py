...
class TestAudioApi:
    ...
    def test_no_audio_uploaded(self, mock_asr: MagicMock, app: Flask) -> None:
        ...
        with (
            app.test_request_context("/audio-to-text", method="POST", data=data, content_type="multipart/form-data"),
    ...
    def test_unsupported_type(self, mock_asr: MagicMock, app: Flask) -> None:
        ...
        with (
            app.test_request_context("/audio-to-text", method="POST", data=data, content_type="multipart/form-data"),
            pytest.raises(UnsupportedAudioTypeError),
        ):
            AudioApi().post(_app_model(), _end_user())
    ...
    def test_provider_not_support(self, mock_asr: MagicMock, app: Flask) -> None:
        ...
        with (
            app.test_request_context("/audio-to-text", method="POST", data=data, content_type="multipart/form-data"),
            pytest.raises(ProviderNotSupportSpeechToTextError),
        ):
            AudioApi().post(_app_model(), _end_user())
...
class TestTextApi:
    ...
    def test_invoke_error_mapped(self, mock_ns: MagicMock, mock_tts: MagicMock, app: Flask) -> None:
        ...
        with app.test_request_context("/text-to-audio", method="POST"), pytest.raises(CompletionRequestError):
