...
def test_skill_inspect_by_agent_returns_strict_json_response():
    ...
    with (
        ...
        response = raw(AgentConfigSkillInspectByAgentApi(), MagicMock(), "tenant-1", _USER, "agent-1", "pdf-toolkit")
...
def test_skill_file_download_by_agent_validates_member_path() -> None:
    ...
    with (
        ...
        body, status = raw(AgentConfigSkillFileDownloadByAgentApi(), MagicMock(), "tenant-1", _USER, "agent-1", "alpha")
    assert status == 404
